...
// - polea: los discos van juntos en la polea (se pueden usar los dos del par);
...
export const EQUIPO_INICIAL = Object.freeze({
  barra: 20,
  polea: 0, // peso propio del carro de la polea: si no se sabe, 0
  ...
  maneral: null, // peso del mango de cada mancuerna, en lb: falta saberlo
  topeMancuerna: 50,
...
/** Tu equipo guardado, completado con lo inicial en lo que falte. */
export function normalizarEquipo(guardado) {
  ...
  return {
    ...
    maneral: esNumero(e.maneral) && e.maneral >= 0 ? e.maneral : null,
    topeMancuerna: esNumero(e.topeMancuerna) && e.topeMancuerna > 0 ? e.topeMancuerna : null,
    discosLb: discos(e.discosLb),
    texto: typeof e.texto === 'string' ? e.texto : EQUIPO_INICIAL.texto,
  };
}
 ...
 * @returns {{implemento:string, peso:number, estado:'exacto'|'aproximado'|'imposible'|'falta-mango',
 *            discos?:number[], cercanos?:number[], minimo?:number, sobreTope?:boolean}}
 *   discos: por lado (barra), por extremo (mancuernas) o todos juntos (polea y landmine).
 */
export function cargar({ implemento, peso, equipo }) {
  ...
  //   (2 lados de la barra o 2 extremos de la mancuerna; 1 en polea y landmine).
  ...
  const armar = (minimo, partes, grupos, disponibles) => {
    ...
    const cercanos = [r.abajo, r.arriba].filter((v) => v !== null).map((v) => Math.round((minimo + v * partes) * 100) / 100);
  ...
  if (implemento === 'polea') return armar(e.polea, 1, 1, e.discosKg);
  if (implemento === 'mancuernas' || implemento === 'mancuerna') {
    if (e.maneral === null) return { ...base, estado: 'falta-mango' };
...
export function pasoDe(implemento, equipo) {
  ...
  let paso = null;
  if (implemento === 'barra') paso = menor(equipo.discosKg, 2) * 2;
  else if (implemento === 'landmine' || implemento === 'polea') paso = menor(equipo.discosKg, 1);
